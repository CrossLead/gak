 ...
 *
 * @param  {Number} Δt_s : Change in time since last last event sent by s
 * @param  {Number} G : Decay constant to weight sensitivity of new events
 * @return {Number}
 */
function g(Δt_s, G) {
  return ( tanh((10*Δt_s)/(G*π) - π) + 1 ) / 2;
}
 ...
 *
 * @param  {Number} Δt_r : time since last event recieved by r ∈ P_i from s
...
export default class EventRank {
  ...
  step( event ) {
    ...
    for (let i = 0; i < nC; i++) {
      ...
      const α = ƒ * Tn * g(Δt_s, G) * h(Δt_r, H);

      if (isParticipent(c)) {
        R_new = R_old + α * ((1 - R_old) / ΣRbar)
      } else {
        R_new = R_old * (1 - (α / Tn));
      }
